...
```
pip install -e .
solrlab doctor            # preflight: docker, ports, disk
solrlab quickstart        # or: quickstart --engine opensearch | elasticsearch
                          # or step by step:
solrlab up --solr-version 9.6 --nodes 2 --heap 1g
solrlab create-collection products --shards 2 --replicas 1
solrlab gen --profile profiles/default.yaml --count 10000 --out data.jsonl
solrlab index --collection products --file data.jsonl --threads 4
solrlab load --collection products --rps 50 --duration 60 --ramp 15 --queries queries/default.yaml
solrlab down
```
...
solrlab is **open-loop**: requests fire on a fixed wall-clock schedule derived from the target RPS. If the server can't keep up, latency climbs and you see it. If the client saturates its in-flight cap, requests are counted as `dropped` instead of silently queued.
...
State lives in `./.solrlab/` (generated compose file + cluster spec), so every command after `up` knows where the cluster is.
...
```
solrlab load --collection products --rps 50 --duration 300 --report chaos-run.json &
solrlab chaos pause solr2       # watch p99 and errors in the timeline
sleep 45
solrlab chaos unpause solr2
solrlab status                  # replica states, recovery
```

## Dashboard

```
solrlab dashboard            # http://localhost:8990, polls the live cluster
solrlab dashboard --demo     # synthesized signals, preview without a cluster
```
...
**Live load-test streaming:** while `solrlab load` runs, it writes rolling stats to `.solrlab/live-load.json` and the dashboard's top panel comes alive automatically — client-observed p50/p99 traces, target vs achieved RPS, progress, errors, dropped. The client-side p99 next to the server-side `/select` p99 is the whole story of a saturation event on one screen: queueing shows up in the client trace before the server metric moves.

## Metrics before/after

```
solrlab metrics --out before.json
solrlab load --collection products --rps 100 --duration 120
solrlab metrics --out after.json
solrlab metrics-diff before.json after.json   # GC ms burned, hitratio drift, merges, adds
solrlab metrics --watch 5                     # or poll live during a run
```
...
```
solrlab drill examples/drill-node-loss.yaml
```
...
(For faults without the orchestration, `solrlab chaos run scenario.yaml` still runs timed steps standalone alongside anything.)
...
solrlab up \
  ...
  --gc-logs \                  # GC logs land in ./.solrlab/gc-logs/<node>
...
```
solrlab load --collection products --rps 100 --duration 300 \
...
```
solrlab up --heap 2g
solrlab create-collection vecs
solrlab schema --collection vecs --profile profiles/vectors.yaml   # DenseVectorField + HNSW knobs
solrlab gen --profile profiles/vectors.yaml --count 200k --out vecs.jsonl
solrlab index --collection vecs --file vecs.jsonl --threads 4      # watch HNSW build cost live
solrlab load --collection vecs --rps 30 --duration 2m --queries queries/vector-solr.yaml
```
...
**Recall — the other half.** Latency without recall is meaningless for ANN; HNSW will happily answer fast and wrong. `recall` computes exact ground truth locally over the dataset you generated (brute force via numpy — `pip install 'solrlab[recall]'`), queries the engine's approximate kNN with in-distribution query vectors, and reports recall@k with latency:

```
solrlab recall --collection vecs --profile profiles/vectors.yaml --data vecs.jsonl \
...
```
solrlab replay --collection products --file solr.log                 # original pacing
solrlab replay --collection products --file solr.log --speed 4       # 4x faster
solrlab replay --collection products --file solr.log --rps 80 --loop 5
solrlab replay --collection products --file solr.log --path-filter /update
```
...
```
solrlab up --heap 512m --gc-logs          # mounts .solrlab/gc-logs/<node>/
# ... run load ...
solrlab gclog                              # all nodes
solrlab gclog solr2                        # one node
solrlab gclog path/to/solr_gc.log          # any unified-logging GC log (JDK 9+)
```
...
```
solrlab k8s > lab.yaml                         # mirrors the running cluster
solrlab k8s --engine os --nodes 3 --heap 2g    # or from flags, no cluster needed
```
...
```
solrlab schema --collection events --profile profiles/high-cardinality.yaml --dry-run
solrlab schema --collection events --profile profiles/high-cardinality.yaml
```
...
```
solrlab sweep examples/heap-sweep.yaml
```
...
```
solrlab load --collection products --rps 100 --duration 2m \
...
```
solrlab up --heap 512m --gc-logs
solrlab create-collection events
solrlab gen --profile profiles/high-cardinality.yaml --count 500000 --out events.jsonl
solrlab index --collection events --file events.jsonl --threads 8
...
**Version regression:** run the identical `gen`/`index`/`load` sequence (same `--seed`) against `--solr-version 8.11.2` and `9.6`, then `solrlab compare v8.json v9.json --html regression.html`.
